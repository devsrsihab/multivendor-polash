                                    ...
                                    @foreach($unique_shops as $key=>$shop)
                                        <div class="chat_list @if ($key == 0) btn--primary @endif"
                                            ...
                                            <div class="chat_people" id="chat_people">
                                                <div class="chat_img">
                                                    <img onerror="this.src='{{asset('assets/front-end/img/image-place-holder.png')}}'" src="{{ $shop->delivery_man_id ?asset('assets/app/delivery-man/'.$shop->image) : asset('assets/appshop/'.$shop->image)}}"
                                                        class="__rounded-10">
                                                </div>
                                                <div class="chat_ib">
                                                ...
                                                @if ($chat->sent_by_seller? $chat->sent_by_seller : $chat->sent_by_delivery_man)
                                                    <div class="incoming_msg">
                                                        <div class="incoming_msg_img"><img
                                                                src="@if($chat->image == 'def.png'){{asset('assets/app'.$chat->image)}} @else {{ $shop->delivery_man_id ?asset('assets/app/delivery-man/'.$last_chat->delivery_man->image) : asset('assets/appshop/'.$last_chat->shop->image)}}
                                                                @endif"
                                                                alt="sunil"></div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $(".seller").click(function (e) {
                        ...
                        if (data.length != 0) {
                            data.map((element, index) => {
                                ...
                                if (element.sent_by_customer) {
                                    ...
                                    let img_path = element.image == 'def.png' ? `{{ asset('assets/appshop') }}/${element.image}` : `{{ (isset($shop->delivery_man_id) && $shop->delivery_man_id) ? asset('assets/app/delivery-man') : asset('assets/appshop') }}/${element.image}`;
                ...
                $(".msg_history").stop().animate({scrollTop: $("#show_msg")[0].scrollHeight}, 1000);
